...
df['Time'] = df['Time'].apply(lambda x: arrow.get(x, 'H:mm:ss.SSSSSS').format('HH:mm:ss'))
print(df.head(8))
...
mustlist = [3, 5, 7]# 必画次数
for n in range(2, 12):
    cols_a = 'Volts Harmonics' + str(n) + ' L1N Avg'
    cols_b = 'Volts Harmonics' + str(n) + ' L2N Avg'
    cols_c = 'Volts Harmonics' + str(n) + ' L3N Avg'
...
for a,b in zip(range(3, 26, 1), ithd_gb):
    coln_a = 'Amps Harmonics' + str(a) + ' L1 Avg'
    coln_b = 'Amps Harmonics' + str(a) + ' L2 Avg'
    coln_c = 'Amps Harmonics' + str(a) + ' L3 Avg'
