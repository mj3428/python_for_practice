...
records4 = (('项目名称', '最大值', '最小值', '95%概率值'),
            ('A相电压有效值', df['Vrms ph-ph L12 Avg'].max(), df['Vrms ph-ph L12 Avg'].min(),
             round(np.percentile(df['Vrms ph-ph L12 Avg'], 95, axis=0),2)),
            ('B相电压有效值', df['Vrms ph-ph L23 Avg'].max(), df['Vrms ph-ph L23 Avg'].min(),
             round(np.percentile(df['Vrms ph-ph L23 Avg'], 95, axis=0),2)),
            ('C相电压有效值', df['Vrms ph-ph L31 Avg'].max(), df['Vrms ph-ph L31 Avg'].min(),
             round(np.percentile(df['Vrms ph-ph L31 Avg'], 95, axis=0),2)))
...
records5 = (('A相电压正偏差', (df['Vrms ph-ph L12 Avg'].max() / 380 - 1) * 100,
             abs(df['Vrms ph-ph L12 Avg'].min() / 380 - 1) * 100,
             abs(np.percentile(df['Vrms ph-ph L12 Avg'], 95, axis=0) / 380 - 1) * 100),
            ('B相电压正偏差', (df['Vrms ph-ph L23 Avg'].max() / 380 - 1) * 100,
                         abs(df['Vrms ph-ph L23 Avg'].min() / 380 - 1) * 100,
             abs(np.percentile(df['Vrms ph-ph L23 Avg'], 95, axis=0) / 380 - 1) * 100),
            ('C相电压正偏差', (df['Vrms ph-ph L31 Avg'].max() / 380 - 1) * 100,
            abs(df['Vrms ph-ph L31 Avg'].min() / 380 - 1) * 100,
             abs(np.percentile(df['Vrms ph-ph L31 Avg'], 95, axis=0)/ 380 - 1) * 100))
...
records6 = (
            ('A相电压谐波总含有率', df['THD V L1N Avg'].max(), df['THD V L1N Avg'].min(), df['THD V L1N Avg'].mean(),
             np.percentile(df['THD V L1N Avg'], 95, axis=0)),
            ('B相电压谐波总含有率', df['THD V L2N Avg'].max(), df['THD V L2N Avg'].min(), df['THD V L2N Avg'].mean(),
             np.percentile(df['THD V L2N Avg'], 95, axis=0)),
            ('C相电压谐波总含有率', df['THD V L3N Avg'].max(), df['THD V L3N Avg'].min(), df['THD V L3N Avg'].mean(),
             np.percentile(df['THD V L3N Avg'], 95, axis=0)))
for i in range(0, 5):
...
for i in Ulist:
    cols_a = 'Volts Harmonics' + str(i) + ' L1N Avg'
    cols_b = 'Volts Harmonics' + str(i) + ' L2N Avg'
    cols_c = 'Volts Harmonics' + str(i) + ' L3N Avg'
...
docx.paragraphs[-1].alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
records7 = (('A相电流有效值', df['Current L1 Avg'].max(), df['Current L1 Avg'].min(), df['Current L1 Avg'].mean(),
            np.percentile(df['Current L1 Avg'], 95, axis=0)),
            ('B相电流有效值', df['Current L2 Avg'].max(), df['Current L2 Avg'].min(), df['Current L2 Avg'].mean(),
            np.percentile(df['Current L2 Avg'], 95, axis=0)),
            ('C相电流有效值', df['Current L3 Avg'].max(), df['Current L3 Avg'].min(), df['Current L3 Avg'].mean(),
            np.percentile(df['Current L3 Avg'], 95, axis=0)))
...
records8 = (('A相电流谐波总含有率', df['THD A L1 Avg'].max(), df['THD A L1 Avg'].min(), df['THD A L1 Avg'].mean(),
             np.percentile(df['THD A L1 Avg'], 95, axis=0)),
            ('B相电流谐波总含有率', df['THD A L2 Avg'].max(), df['THD A L2 Avg'].min(), df['THD A L2 Avg'].mean(),
             np.percentile(df['THD A L2 Avg'], 95, axis=0)),
            ('C相电流谐波总含有率', df['THD A L3 Avg'].max(), df['THD A L3 Avg'].min(), df['THD A L3 Avg'].mean(),
             np.percentile(df['THD A L3 Avg'], 95, axis=0)))
...
for i in ilist:
    coln_a = 'Amps Harmonics' + str(i) + ' L1 Avg'
    coln_b = 'Amps Harmonics' + str(i) + ' L2 Avg'
    coln_c = 'Amps Harmonics' + str(i) + ' L3 Avg'
    trans_arrray0 = np.array(df[coln_a] * df['Current L1 Avg'])
    trans_arrray1 = np.array(df[coln_b] * df['Current L2 Avg'])
    trans_arrray2 = np.array(df[coln_c] * df['Current L3 Avg'])
...
records9 = (('A相功率因数', df['Power Factor L1N Avg'].max(), df['Power Factor L1N Avg'].min(), df['Power Factor L1N Avg'].mean(),
            np.percentile(df['Power Factor L1N Avg'], 95, axis=0)),
            ('B相功率因数', df['Power Factor L2N Avg'].max(), df['Power Factor L2N Avg'].min(), df['Power Factor L2N Avg'].mean(),
            np.percentile(df['Power Factor L2N Avg'], 95, axis=0)),
            ('C相功率因数', df['Power Factor L3N Avg'].max(), df['Power Factor L3N Avg'].min(), df['Power Factor L3N Avg'].mean(),
            np.percentile(df['Power Factor L3N Avg'], 95, axis=0)),
            ('合计', df['Power Factor Total Avg'].max(), df['Power Factor Total Avg'].min(), df['Power Factor Total Avg'].mean(),
            np.percentile(df['Power Factor Total Avg'], 95, axis=0)))
