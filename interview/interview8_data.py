...
基于数据库实现商城商品计数？
计数器表在WEB应用中很常见。可以用这种表缓存一个用户的朋友书、文件下载次数等。
创建一张独立的表存储计数器通常是一个好主意，这样可使计数器表小且快。使用独立的表可以帮助避免查询缓存失效。
...
问题在于，对于任何想要更新这一行的事务来说，这条记录上都有一个全局的互斥锁(MUTEX)。这会使得这些事务只能串行执行。
要获得更高的并发更新性能，有可以将计数器保存在多行记录中，每次随机选择一行进行更新。这样做需要对计数器表进行如下修改：
mysql>CREATE TABLE hit_counter(
...
答案：https://www.cnblogs.com/wupeiqi/articles/5748496.html（题主给的答案）
      https://www.cnblogs.com/xone/p/6249353.html(部分题)
