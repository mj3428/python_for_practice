...
docx.add_paragraph('本次测试采用C.A 8335系列电能质量和能量进行测试。设备参数如下表：')
docx.add_picture('./detect_pic/8335_ES.png', height=Cm(8.55), width=Cm(7.26))
docx.paragraphs[-1].alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
pic1 = docx.add_paragraph().add_run('图1：电能质量分析仪 C.A 8335')
docx.paragraphs[-1].alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
pic1.font.bold = True
...
records4 = (('项目名称', '最大值', '最小值', '95%概率值'),
            ('A相电压有效值', round(df0['U1 RMS'].max(),2), round(df0['U1 RMS'].min(),2),
             round(np.percentile(df0['U1 RMS'], 95, axis=0),2)),
            ('B相电压有效值', round(df0['U2 RMS'].max(),2), round(df0['U2 RMS'].min(),2),
             round(np.percentile(df0['U2 RMS'], 95, axis=0),2)),
            ('C相电压有效值', round(df0['U3 RMS'].max(),2), round(df0['U3 RMS'].min(),2),
             round(np.percentile(df0['U3 RMS'], 95, axis=0),2)))
...
records5 = (('A相电压正偏差', (df0['U1 RMS'].max() / 380 - 1) * 100,
             abs(df0['U1 RMS'].min() / 380 - 1) * 100,
             round(abs(np.percentile(df0['U1 RMS'], 95, axis=0) / 380 - 1) * 100,2)),
            ('B相电压正偏差', (df0['U2 RMS'].max() / 380 - 1) * 100,
                         abs(df0['U2 RMS'].min() / 380 - 1) * 100,
             round(abs(np.percentile(df0['U2 RMS'], 95, axis=0) / 380 - 1) * 100,2)),
            ('C相电压正偏差', (df0['U3 RMS'].max() / 380 - 1) * 100,
            abs(df0['U3 RMS'].min() / 380 - 1) * 100,
             round(abs(np.percentile(df0['U3 RMS'], 95, axis=0)/ 380 - 1) * 100,2)))
...
records6 = (
            ('A相电压谐波总含有率', df0['V1 THD'].max(), df0['V1 THD'].min(), df0['V1 THD'].mean(),
             round(np.percentile(df0['V1 THD'], 95, axis=0),2)),
            ('B相电压谐波总含有率', df0['V2 THD'].max(), df0['V2 THD'].min(), df0['V2 THD'].mean(),
             round(np.percentile(df0['V2 THD'], 95, axis=0),2)),
            ('C相电压谐波总含有率', df0['V3 THD'].max(), df0['V3 THD'].min(), df0['V3 THD'].mean(),
             round(np.percentile(df0['V3 THD'], 95, axis=0),2)))
for i in range(0, 5):
...
for i in Ulist:
    ...
    minValue = np.min([df1[cols_a], df1[cols_b], df1[cols_c]])
    meanValue = np.mean([df1[cols_a], df1[cols_b], df1[cols_c]])
...
docx.paragraphs[-1].alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
records7 = (('A相电流有效值', df0['A1 RMS'].max(), df0['A1 RMS'].min(), df0['A1 RMS'].mean(),
            round(np.percentile(df0['A1 RMS'], 95, axis=0),2)),
            ('B相电流有效值', df0['A2 RMS'].max(), df0['A2 RMS'].min(), df0['A2 RMS'].mean(),
            round(np.percentile(df0['A2 RMS'], 95, axis=0),2)),
            ('C相电流有效值', df0['A3 RMS'].max(), df0['A3 RMS'].min(), df0['A3 RMS'].mean(),
            round(np.percentile(df0['A3 RMS'], 95, axis=0),2)))
...
records8 = (('A相电流谐波总含有率', df0['A1 THD'].max(), df0['A1 THD'].min(), df0['A1 THD'].mean(),
             np.percentile(df0['A1 THD'], 95, axis=0)),
            ('B相电流谐波总含有率', df0['A2 THD'].max(), df0['A2 THD'].min(), df0['A2 THD'].mean(),
             np.percentile(df0['A2 THD'], 95, axis=0)),
            ('C相电流谐波总含有率', df0['A3 THD'].max(), df0['A3 THD'].min(), df0['A3 THD'].mean(),
             np.percentile(df0['A3 THD'], 95, axis=0)))
...
for i in ilist:
    ...
    maxValue = np.max([trans_arrray0, trans_arrray1, trans_arrray2])
    minValue = np.min([trans_arrray0, trans_arrray1, trans_arrray2])
    meanValue = np.mean([trans_arrray0, trans_arrray1, trans_arrray2])
...
records9 = (('A相功率因数', df0['PF1'].max(), df0['PF1'].min(), df0['PF1'].mean(),
            round(np.percentile(df0['PF1'], 95, axis=0),2)),
            ('B相功率因数', df0['PF2'].max(), df0['PF2'].min(), df0['PF2'].mean(),
            round(np.percentile(df0['PF2'], 95, axis=0),2)),
            ('C相功率因数', df0['PF3'].max(), df0['PF3'].min(), df0['PF3'].mean(),
            round(np.percentile(df0['PF3'], 95, axis=0),2)),
            ('合计', df0['PF 平均'].max(), df0['PF 平均'].min(), df0['PF 平均'].mean(),
            round(np.percentile(df0['PF 平均'], 95, axis=0),2)))
